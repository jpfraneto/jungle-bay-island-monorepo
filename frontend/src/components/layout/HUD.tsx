...
function viewHint(
  viewMode: ReturnType<typeof useIslandStore.getState>["viewMode"],
): string {
...
export const HUD = memo(function HUD() {
  ...
  const selectedTicker = selectedBungalow
    ? BUNGALOW_BY_ID.get(selectedBungalow)?.ticker
    : undefined;
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-20">
      <div className="absolute left-4 top-4 rounded-2xl border border-white/10 bg-black/30 px-4 py-3 backdrop-blur-md">
        ...
        {selectedTicker ? (
          <p className="font-mono text-xs uppercase tracking-[0.2em] text-emerald-200">
            {selectedTicker}
          </p>
        ) : null}
      </div>
      ...
      <div className="absolute bottom-4 left-4 rounded-2xl border border-white/10 bg-black/30 px-4 py-3 backdrop-blur-md">
        ...
        <p className="font-display text-xl text-emerald-200">
          {walletAddress
            ? formatHeat(userQuery.data?.island_heat)
            : "Connect wallet"}
        </p>
      </div>
