...
const seededCulture: Record<string, string> = {
  JBM: "Jungle Bay began as campfire signal for builders on Base, mixing identity and utility.",
...
export function Wall({
  ...
  const seeded = HOME_TEAM_TICKERS.includes(bungalow.token_symbol)
    ? seededCulture[bungalow.token_symbol] ||
      "Home Team token with deep island lore and active cultural stewards."
    : undefined;
    ...
    <section className="card space-y-3">
      ...
      {canEdit && onSaveOriginStory ? (
        ...
        <p className="text-sm text-zinc-400">
          This bungalow is waiting for its origin story. Claim this token and
          add the culture, history, and visual identity.
        </p>
      )}
    </section>
  );
