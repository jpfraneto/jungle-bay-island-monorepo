...
function fallbackDescription(
  ...
  if (name && symbol) {
    return `${name} (${symbol}) on ${chainLabel(chain)} — view heat, holders, and bungalow activity on Memetics.`;
  }
  if (name) {
    return `${name} on ${chainLabel(chain)} — view heat, holders, and bungalow activity on Memetics.`;
  }
  return `View token ${tokenAddress} on ${chainLabel(chain)} in Memetics.`;
}
...
export async function resolveTokenMetadata(
  ...
  const needsSolanaFallback =
    chain === "solana" &&
    needsDexFallback &&
    (!dexData?.tokenName || !dexData?.imageUrl);
  const solMeta = needsSolanaFallback
    ? await fetchSolanaTokenMetadata(tokenAddress)
    : null;
  ...
  const description =
    ...
    solMeta?.description ??
    fallbackDescription(chain, tokenAddress, name, symbol);
  const image_url =
    bungalow?.image_url ?? dexData?.imageUrl ?? solMeta?.image ?? null;
  ...
  const market_data =
    ...
    dexData?.marketCap
      ? {
          price_usd: bungalow?.price_usd
            ? Number(bungalow.price_usd)
            : (dexData?.priceUsd ?? null),
          market_cap: bungalow?.market_cap
            ? Number(bungalow.market_cap)
            : (dexData?.marketCap ?? null),
          volume_24h: bungalow?.volume_24h
            ? Number(bungalow.volume_24h)
            : (dexData?.volume24h ?? null),
          liquidity_usd: bungalow?.liquidity_usd
            ? Number(bungalow.liquidity_usd)
  ...
  const resolved: ResolvedTokenMetadata = {
    ...
    links: {
      ...
      website:
        bungalow?.link_website ??
        dexData?.linkWebsite ??
        solMeta?.website ??
        null,
      dexscreener:
        bungalow?.link_dexscreener ?? dexData?.linkDexscreener ?? null,
    },
  };
